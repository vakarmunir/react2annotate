...
import {Grid , Row , Col , Panel , Well , FormControl , FormGroup , InputGroup , Button , ButtonToolbar , Alert} from 'react-bootstrap';
    ...
    app.include((options) => {
      return {
          annotationCreated: (annotation) => {                
            onAddedAnnotationRef(annotation.id , annotation.quote,annotation.text);
          },
          annotationDeleted: (annotation) => {                
            onDeletedAnnotationRef(annotation.id);
          },
          annotationUpdated: (annotation) => {                
            onUpdatedAnnotationRef(annotation.id , annotation.quote,annotation.text);
          }
      };
    });
        ...
        <Grid>
          <Row>    
            <Col xs={12} md={9}>
              <form>
                <ButtonToolbar>
                  <Button bsStyle="primary" onClick={(e) => {this.setState({nav:true})}}>Load Html</Button>
                    &nbsp;
                  <Button bsStyle="success" onClick={(e) => {this.setState({nav:false})}}>Load PDF</Button>
                </ButtonToolbar>              
              </form>             
            </Col>
          </Row>
